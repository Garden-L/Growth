...
## 부트로더
### 1. 개념
컴퓨터에 전원이 들어오면 ROM에 저장되어 있는 바이오스가 전체적인 시스템을 초기화 시키고 바이오스에 설정되어있는 부팅 순서대로 부트로더들을 불러온다. 하드 디스크가 첫번째 부팅 장치로 설정되어 있다면 하드디스크에서 앞 512bytes 데이터를 불러온다. 개발자는 512bytes 프로그램으로 커널을 실행시킬 코드(부트로더)를 작성해야 안정적으로 시스템을 적재할 수 있다. 부트로더가 커널소스를 적재하면 모든 제어권을 커널에게 넘어가게된다.

### 2. 부트로더의 위치
ROM에서 부트로더를 적재하기 위해 보조메모리의 앞 512바이트에서 불러온다고 설명했다. 그럼 부트로더는 OS가 설치된 디스크의 0번 섹터에 앞 0~511번째를 차지하고 있어야 될 것이다. 이 섹터 공간을 MBR(Master Boot Record)이라 부른다. 여기에는 기계어로 번역된 파일이 저장되어 있어야한다. 반드시 섹터 0번이 MBR이 아닐 수도 있다. 왜냐하면 외장하드 같은 OS가 설치되지 않은 디스크는 부트섹터가 없기 때문이다. ROM에서는 부트로더인지 확인하기 위해 511, 512 바이트에 0xAA55가 기록되어있는지 확인하고 기록이 되어 있으면 부트로더라고 판단한다. 인식된 부트로더는 512bytes를 메모리 0x07C0에 적재한다. 리눅스에서는 커널에 관한 모든 코드의 압축을 해제하고 메모리에 적재시키는 코드가 부트로더에 입력되어있다.

### 3. 2차 부트로더
과거의 커널 소스가 길지가 않아 충분히 512바이트로 부팅소스를 실행시킬 수 있었지만 현대에서는 하나에 보조메모리에 여러 운영체제를 올리기도하고 운영체제의 소스코드가 길어짐으로 인해 512바이트의 부트로더로는 부족해졌다. 그래서 2차 부트로더가 생겨났다.
