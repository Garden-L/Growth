...
### 추상팩토리 패턴이란?
추상 팩토리 패턴은 다른 팩토리를 만드는 super-factory이다. factory of factories 라고도 불린다. 

### 추상 팩토리를 사용해야 하는 경우?
팩토리는 특정 인스턴스를 생성하는 하나의 객체이다. 추상 팩토리는 팩토리를 생성하는 팩토리이다. 이런 어떤 경우에 사용될까? 만약 컴퓨터를 파는 회사가 있다고 하자. 컴퓨터의 구성품은 본체, 키보드, 마우스이다. 본체, 키보드, 마우스가 팩토리 클래스라고 하자. 각각 제조사는 LG, 삼성이 있다. 컴퓨터를 파는 회사는 LG대리점, 삼성 대리점이 있다. 삼성 대리점에서는 모든 부품이 삼성제품이여야 하며, LG대리점은 모든 부품이 LG 제품이여야 한다. 컴퓨터를 팔기 위해 제품 구성 클래스를 만든다. 이를 삼성 대리점, LG대리점에서 상속받고 각각의 본체 키보드 마우스를 구성하여 판매한다. 이때 제품 구성 클래스는 추상 팩토리가 된다. 본체 키보드 마우스가 모두 팩토리 클래스이기 때문에 부품들을 구성하여 팔기위해 추상 팩토리로 제품 구성 클래스를 만드는 것이다. 
